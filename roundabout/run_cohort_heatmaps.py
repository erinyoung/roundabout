...
def run_cohort_heatmaps(
    ...
    local_matrix_df: pd.DataFrame, # <-- We need this back for the local-only map!
    ...
    """
    heatmap_base_dir = Path(outdir) / "ani_heatmap_results"
    ...
    for group_id, members in pipeline_groups.items():
        ...
        if len(local_ids) >= 2:
            ...
            local_out_dir = heatmap_base_dir / group_id / "local_only"
            local_out_dir.mkdir(parents=True, exist_ok=True)
            local_heatmap_path = local_out_dir / f"{group_id}_local_ani_heatmap.png"
            
            visualize_as_heatmap(matrix_df=local_group_matrix, out_path=local_heatmap_path)
        else:
        ...
        for member in members:
            ref_paths = sample_to_ref_paths.get(member) or sample_to_ref_paths.get(clean_id(member))
            if ref_paths:
        ...
        if cohort_rows.empty:
            logging.warning(f"{group_id}: No matching RefSeq alignments found in global data.")
            continue
        ...
        refseq_out_dir = heatmap_base_dir / group_id / "with_refseq"
