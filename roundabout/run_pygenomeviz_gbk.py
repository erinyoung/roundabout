...
def _normalize_and_sort_gbks(
    gbk_paths: list[Path], local_matrix_df: pd.DataFrame
) -> list[Genbank]:
    """
    ...
    for gbk in raw_gbks:
        filename_stem = (
            Path(gbk.file_path).stem if hasattr(gbk, "file_path") else gbk.name
        )
...
def _render_links_and_save(
    gv: GenomeViz, align_coords: list, out_path: Path, has_identity: bool
):
    """
...
def run_pygenomeviz_gbk_blast(
    ...
    try:
        ...
        align_coords = AlignCoord.filter(
            align_coords, length_thr=min_length, identity_thr=min_identity
        )
...
def run_pygenomeviz_gbk_mummer(
    gbk_paths: list[Path],
    local_matrix_df: pd.DataFrame,
    out_path: Path,
    min_length: int = 100,
...
def run_pygenomeviz_gbk_pmauve(
    gbk_paths: list[Path],
    local_matrix_df: pd.DataFrame,
    out_path: Path,
    min_length: int = 100,
...
def run_pygenomeviz_gbk_mmseqs(
    gbk_paths: list[Path],
    local_matrix_df: pd.DataFrame,
    out_path: Path,
    min_length: int = 100,
