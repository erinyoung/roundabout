...
def visualize_global_matches_scatter(global_df: pd.DataFrame, out_path: Path):
    ...
    if 'organism' in global_df.columns:
        ...
        has_taxa = False
        logging.getLogger(__name__).warning("Organism column missing. Defaulting to single color.")
    ...
    sns.stripplot(
        ...
        palette='tab10' if has_taxa else None, # 'tab10' perfectly matches our 10 discrete categories
    ...
    ax.axvline(x=95.0, color='black', linestyle='--', alpha=0.5, label='95% ANI Threshold')
