...
def generate_amr_highlights_from_tsv(
    reference_id: str, target_amr_gene: str, amr_outdir: Path, out_csv: Path
) -> bool:
    """
    ...
    if not amr_tsv.exists():
        logging.warning(
            f"AMRFinder TSV not found at {amr_tsv}. Cannot generate highlights."
        )
        return False
    ...
    try:
        with open(amr_tsv, "r") as tsvfile, open(out_csv, "w", newline="") as csvfile:
            ...
            for row in reader:
                ...
                if target:
                    if (
                        target in element_symbol.lower()
                        or target in element_name.lower()
                    ):
                        is_match = True
                else:
                    is_match = True
...
def execute_minkemap_run(
    central_fasta: Path,
    query_fastas: list,
    out_dir: Path,
    out_prefix: str,
    minkemap_opts: dict,
    highlight_csv: Path = None,
):
    """
...
def run_minkemap_cohorts(
    ...
    for group_id, members in pipeline_groups.items():
        logging.info(
            f"Processing MinkeMap visualizations for {group_id} ({len(members)} members)..."
        )
        ...
        for central_id in members:
            ...
            local_queries = [
                fasta_map[q_id]
                for q_id in members
                if q_id != central_id and q_id in fasta_map
            ]
            ...
            sample_root = Path(outdir) / group_id / "minkemap_results" / central_id
            ...
            # -----------------------------------------------------------------
            logging.info(
                f"Processing MinkeMap visualizations for {central_id} in {group_id}"
            )
            # standard flavor
            ...
            if generate_amr_highlights_from_tsv(
                central_id, target_amr_gene, amr_outdir, local_highlight_csv
            ):
                logging.info(
                    f"Adding AMR highlights to MinkeMap visualizations for {central_id} in {group_id}"
                )
                execute_minkemap_run(
                    central_fasta=central_fasta,
                    query_fastas=local_queries,
