...
def check_bakta_db(db_dir: str):
    ...
    try:
        with open(version_file, "r") as f:
            data = json.load(f)
            logging.info(
                f"Success: Found Bakta '{data.get('type', 'unknown')}' database v{data.get('major', '?')}.{data.get('minor', '?')}"
            )
        return db_dir
...
def check_plasmidfinder_db(db_dir: str):
    ...
    if (
        ...
        or not os.path.exists(version_file)
    ):
...
def check_refseq_plasmid_dl_db(db_dir: str):
    ...
    missing_files = [
        os.path.basename(f)
        for f in [final_fasta, metadata_csv, report_csv]
        if not os.path.exists(f)
    ]
    if missing_files:
        logging.error(
            f"Missing refseq database files in {db_dir}: {', '.join(missing_files)}"
        )
        return None
        ...
        with open(report_csv, "r") as f:
            for line in f:
                if line.startswith("Sequences Written to FASTA"):
                    ...
                    logging.info(
                        f"Success: Found refseq Plasmid database ({seq_count} sequences)"
                    )
                    break
        return db_dir
    except Exception as e:
...
def setup_amrfinder_database(target_dir: str = "", force: bool = False):
    ...
    logging.info(
        f"Downloading AMRFinderPlus database to {target_dir or 'default location'}"
    )
...
def setup_plasmidfinder_database(target_dir, force: bool = False):
    ...
    logging.info(
        f"Setting up PlasmidFinder database at {target_dir or 'default location'}"
    )
    ...
    if os.path.exists(db_dir) and os.listdir(db_dir):
        ...
        if not force:
            logging.info(
                "Skipping download and indexing since --force flag is not set."
            )
            return db_dir
        else:
            logging.info(
                "Force flag detected. Removing existing PlasmidFinder DB for update."
            )
            shutil.rmtree(db_dir, ignore_errors=True)
    ...
    try:
        ...
        subprocess.run(
            index_cmd, cwd=db_dir, check=True, capture_output=True, text=True
        )
...
def setup_refseq_plasmid_dl_database(target_dir: str, force: bool = False) -> str:
    ...
    logging.info(
        f"Setting up refseq Plasmid database at {target_dir or 'default location'}"
    )

    # Resolve the target directory
    db_dir = (
        target_dir if target_dir else os.path.join(os.getcwd(), "refseq_plasmids_db")
    )
    final_fasta = os.path.join(db_dir, "refseq_plasmids_dl.fasta")
    ...
    try:
        logging.info(
            "Running refseq-plasmid-dl (this may take a while depending on NCBI servers)..."
        )
        ...
        logging.error(
            f"Failed to setup refseq Plasmid database. Error code: {e.returncode}"
        )
        raise
...
def run_setup(args):
    ...
    if not args.skip_db:
        ...
        plasmidfinder_db = setup_plasmidfinder_database(
            args.plasmidfinder_db, args.force
        )
    ...
    if not args.skip_db:
        ...
        refseq_plasmid_dl_db = setup_refseq_plasmid_dl_database(
            args.refseq_plasmid_dl_db, args.force
        )
    refseq_plasmid_dl_db = check_refseq_plasmid_dl_db(refseq_plasmid_dl_db)

    # Final Validation
    if not args.skip_db and not all(
        [amr_db, bakta_db, plasmidfinder_db, refseq_plasmid_dl_db]
    ):
        logging.error(
            "One or more databases failed to set up properly. Please check the logs for details."
        )
        sys.exit(1)
