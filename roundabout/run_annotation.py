...
# AMRFinderPlus
# -----------------------------------------------------------------------------
def _run_single_amrfinder(
    fasta_path: Path, outdir: Path, db_path: str, opts: dict
) -> Path:
...
def execute_amrfinder_parallel(
    fasta_paths: list[Path], outdir: Path, db_path: str, threads: int, opts: dict
) -> dict[str, list[str]]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        ...
        futures = {
            executor.submit(_run_single_amrfinder, f, amr_outdir, db_path, opts): f
...
def _run_single_plasmidfinder(
    ...
    try:
        ...
        logging.error(
            f"PlasmidFinder crashed on {fasta_path.name}. PF Error:\n{e.stderr}\n{e.stdout}"
        )
        raise


def execute_plasmidfinder_parallel(
    fasta_paths: list[Path], outdir: Path, db_path: str, threads: int, opts: dict
) -> dict[str, list[str]]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
        futures = {
            executor.submit(_run_single_plasmidfinder, f, pf_outdir, db_path, opts): f
            for f in fasta_paths
        }
...
def _run_single_bakta(fasta_path: Path, outdir: Path, db_path: str, opts: dict) -> Path:
    ...
    try:
        ...
        logging.error(
            f"Bakta crashed on {fasta_path.name}. Bakta Error:\n{e.stderr}\n{e.stdout}"
        )
        raise


def execute_bakta_parallel(
    fasta_paths: list[Path], outdir: Path, db_path: str, threads: int, opts: dict
) -> list[Path]:
    ...
    # 1. Separate files that already have completed results
    for f in fasta_paths:
        # Match Bakta's subfolder and naming convention (e.g., prefix.gbff)
        expected_subdir = bakta_outdir / f.stem
        expected_gbff = expected_subdir / f"{f.stem}.gbff"

        if expected_gbff.exists():
            logging.info(f"Found existing gbff file for {f.name}, skipping Bakta run.")
            completed.append(expected_subdir)
        else:
            to_run.append(f)

    # 2. Only spin up threads for sequences that actually need to be analyzed
    if to_run:
        with concurrent.futures.ThreadPoolExecutor(max_workers=threads) as executor:
            futures = {
                executor.submit(_run_single_bakta, f, bakta_outdir, db_path, opts): f
                for f in to_run
            }
    ...
    return sorted(list(bakta_outdir.glob("*/[!.]*.gbff")))
