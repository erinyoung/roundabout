...
def execute_skani(
    ...
    if skani_tsv.exists():
        logging.info(
            "Found existing Skani matrix file. Skipping skani identity profiling."
        )
        return skani_tsv
    ...
    # Check if any preset is currently active
    has_preset = any(
        [
            opts.get("fast"),
            opts.get("medium"),
            opts.get("slow"),
            opts.get("small_genomes"),
        ]
    )
    ...
    for key, flag in val_mapping.items():
        # If a preset is active, skip injecting manual compression factors
        if key in ["c", "m"] and has_preset:
            continue
...
def create_local_ani_matrix(
    skani_df: pd.DataFrame, input_fastas: list[Path]
) -> pd.DataFrame:
    """
    ...
    q_col = (
        "Query_file" if "Query_file" in cols else ("query" if "query" in cols else None)
    )
    r_col = (
        "Ref_file" if "Ref_file" in cols else ("target" if "target" in cols else None)
    )
    ...
    matrix_df = local_df.pivot(
        index="query_name", columns="target_name", values=ani_col
    )
...
def extract_global_hits(
    skani_df: pd.DataFrame,
    input_fastas: list[Path],
    refseq_meta_csv: Path | None = None,
) -> pd.DataFrame:
    """
    ...
    q_col = (
        "Query_file"
        if "Query_file" in cols
        else ("query" if "query" in cols else cols[0])
    )
    r_col = (
        "Ref_file"
        if "Ref_file" in cols
        else ("target" if "target" in cols else cols[1])
    )
    ...
    if not global_df.empty:
        ...
        def extract_accession(val):
            seq_id = str(val).split()[
                0
            ]  # Grab just the accession, drop the description
        ...
        if refseq_meta_csv and refseq_meta_csv.exists():
            try:
                ...
                global_df = global_df.merge(
                    meta_df, how="left", left_on="RefSeq_Hit", right_on="accession"
                )
        ...
        global_df = global_df.sort_values(
            by=["Query_Name", ani_col], ascending=[True, False]
        )

        # Reorder columns to put the cleaned names first
        cols_to_keep = ["Query_Name", "RefSeq_Hit"] + [
            c
            for c in global_df.columns
            if c not in ["Query_Name", "RefSeq_Hit", q_col, r_col, ref_name_col]
        ]
